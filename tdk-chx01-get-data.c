...
#define FIRMWARE_PATH           "/home/chirp-sensor-v3/tdk-chx01-get-data-app/files/firmware/"
...
int getDataOnce(int * res,int size){
        char *buffer;
        int total_bytes;
        struct pollfd pfds[1];
        long long timestamp, last_timestamp;
...
int main(int argc, char *argv[])
{
        ...
        // while(true){
                // int size = 500;
                // int* userArray = (int*)malloc(size * sizeof(int));
                // for(int i =0; i<size; i++){
                //         userArray[i] = -1;
                // }
                // if(getDataOnce(userArray,size) < 0){
                //         printf("user allocated array too small for data\n");
                // }
                // for(int i=1; i<size; i++){
                //         if(userArray[i] == -1){
                //                 break;
                //         }
                //         printf("Sensor: %d -> %d\n",(((i-1)%userArray[0]) + 1),userArray[i]);
                // }
        // }
        getData();
        stopData();
